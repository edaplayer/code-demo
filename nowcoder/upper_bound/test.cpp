...
int lower_bound_(vector<int> &a, int n, int target) {
	...
	while (l < r) {
		mid = (l + r) >> 1; //中点 = (l+r)/2
		if (a[mid] < target)
			l = mid + 1; //如果中点元素小于目标，说明中点左侧数据均不符合，左坐标可以改为中点坐标+1
		else
			r = mid; //如果中点元素不小于目标，说明目标值位于中点左侧（包括中点），把右坐标改为中点位置
		// sleep(1);
	}
...
int upper_bound_(vector<int> &a, int size, int target) {
	if (a.back() <= target) return size + 1;
	...
	while(l < r) {
		...
		// cout << "mid = " << mid << endl;
		if(a[mid] <= target)
			l = mid + 1;//如果中点元素大于或等于目标值，说明目标值在右侧，左坐标移动到中点坐标+1
		else
...
int binary_search(vector<int> &a, int n, int target) {
	if (a.back() < target || a.front() > target) return n + 1; //超出范围，返回n + 1
...
int main()
{
	...
	int res = lower_bound_(data, data.size(), 5);
